...
import { registerTravelUpcomingTools } from '../tools/travel-upcoming.js';
...
export function createOnflyMcpServer(): McpServer {
  const server = new McpServer(
    ...
    },
    {
      instructions: [
        ...
        "TRAVEL: For upcoming / future trips (next trips, próximas viagens), call list_my_upcoming_trips — it merges fly/hotel/bus/auto for the authenticated user with start_date defaulting to today (UTC). For past or recent personal bookings (e.g. my last flight, my trips, qual meu ultimo voo), call list_my_travel_orders — it filters by the authenticated user automatically. Never use company-wide list_travel_orders for personal queries; it returns 3000+ records from all employees and cannot efficiently answer per-user questions.",
        "EXPENSES/RDV: To open a brand-new travel expense report (RDV), use create_rdv (title, cost_center_id; optional reason; for trip dates use start_trip_date and end_trip_date as YYYY-MM-DD so the API receives startTripDate/endTripDate and manual trip flags). For questions about the user's own last trip or latest RDV, call get_my_latest_rdv first instead of guessing from company-wide list_rdvs. To add an expense dated on that trip and linked to its RDV, prefer create_expense_on_latest_trip (amount_brl). If you already have date, rdv_id, and ids from context, create_expense works too: use amount_brl: 110 OR amount: 11000 (centavos), never omit both. Always read the tool JSON: success false means the expense was not created - quote error, hint, and next_step to the user verbatim when present, and suggest fixes (RDV closed, wrong cost center, etc.).",
  ...
  registerTravelUpcomingTools(server, sharedOnflyLimiter);
